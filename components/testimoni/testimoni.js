...
import {
  ...
  query, where, onSnapshot, deleteDoc, updateDoc, orderBy
...
async function kirimTestimoni() {
  ...
  let myInvoices = JSON.parse(localStorage.getItem("myTestimoni")) || [];
...
function renderTestimoni() {
  ...
  let avg = total ? (totalRating / total).toFixed(1) : "0.0";

  document.getElementById("avgRating").innerText = avg;
  document.getElementById("totalReview").innerText = total + " ulasan pelanggan";
    ...
    container.innerHTML += `
      <div class="preview-testimoni-card">
        ...
        <div class="preview-testimoni-user">
          <div class="preview-testimoni-avatar">${avatar}</div>
          <div>
...
window.toggleText = function(id) {
  ...
  setTimeout(() => {
    semuaTestimoni.forEach(t => {
      ...
      let btnToggle = document.getElementById("toggle-" + t.id);
...
window.showInvoiceInvalid = function() {
  document.getElementById("invoiceInvalidModal").classList.add("show");
};

window.tutupInvoiceInvalid = function() {
  document.getElementById("invoiceInvalidModal").classList.remove("show");
};
...
window.editTestimoni = function(id) {
  ...
  let titik = document.querySelector("[onclick=\"toggleMenu('" + id + "')\"]");
  if (titik) titik.style.display = "none";

  setTimeout(() => {
    document.getElementById("edit-" + id).closest(".testimoni-card").scrollIntoView({
      behavior: "smooth",
      block: "center"
    });
  }, 150);
  ...
  let editStars = document.querySelectorAll("#editStars-" + id + " span");
  editStars.forEach(star => {
    star.onclick = function() {
      let value = this.dataset.value;
      editRating[id] = value;

      editStars.forEach(s => s.style.color = "#d1d5db");
      for (let i = 0; i < value; i++) {
        editStars[i].style.color = "#fbbf24";
      }
    };
  });
  ...
  let counterEdit = document.getElementById("charCounter-" + id);
  counterEdit.style.display = "block";
  ...
  textarea.addEventListener("input", function() {
    ...
    counterEdit.innerText = "Maks: " + sisa + " karakter";
    counterEdit.classList.remove("counter-normal", "counter-warning", "counter-danger");

    if (sisa >= 201) {
      counterEdit.classList.add("counter-normal");
    } else if (sisa >= 51) {
      counterEdit.classList.add("counter-warning");
    } else {
      counterEdit.classList.add("counter-danger");
    }
  });
};
...
function cancelEdit(id) {
  ...
  let counterEdit = document.getElementById("charCounter-" + id);
  if (counterEdit) counterEdit.style.display = "none";
...
const mainCounter = document.getElementById("charCounter");

if (pesanInput && mainCounter) {
  pesanInput.addEventListener("input", function() {
    let max = 300;
    let sisa = max - this.value.length;

    mainCounter.innerText = "Maks: " + sisa + " karakter";
    mainCounter.classList.remove("counter-normal", "counter-warning", "counter-danger");

    if (sisa >= 201) {
      mainCounter.classList.add("counter-normal");
    } else if (sisa >= 51) {
      mainCounter.classList.add("counter-warning");
    } else {
      mainCounter.classList.add("counter-danger");
    }

    if (sisa <= 10) {
      mainCounter.style.transform = "scale(1.1)";
      setTimeout(() => {
        mainCounter.style.transform = "scale(1)";
      }, 150);
    }
  });
